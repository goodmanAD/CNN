...
prob_test2 = np.concatenate((prob_train_Hi,prob_train_Lo),axis=0)
prob_test2 = prob_test2[:,0]
...
#np.savetxt('fpr_dylf_trainLF.txt',fpr,delimiter=" ")
#np.savetxt('tpr_dylf_trainLF.txt',tpr,delimiter=" ")
...
fpr1, tpr1, _1 = roc_curve(val_class2,prob_test2)
auc_score1 = auc(fpr1,tpr1)
#np.savetxt('fprtrain_dytthf_trainHF.txt',fpr1,delimiter=" ")
#np.savetxt('tprtrain_dytthf_trainHF.txt',tpr1,delimiter=" ")

print('The AUC is',auc_score)
print('The train AUC is',auc_score1)


testh = plt.hist(prob_test,bins=np.arange(0,1.0,0.05),log=False)
testhe = np.sqrt(testh[0])

obshdy = plt.hist(prob_test_Hi[:,0],bins=np.arange(0,1.0,0.05), log=False)
obshdye = np.sqrt(obshdy[0])
obshqcd = plt.hist(prob_test_Lo[:,0],bins=np.arange(0,1.0,0.05), log=False)
obshqcde = np.sqrt(obshqcd[0])


plt.figure(figsize=(8,6))
plt.errorbar(obshdy[1][1:]-0.025, obshdy[0], yerr=obshdye, fmt='.', color="xkcd:green",label="Test Hi", markersize='10')
plt.errorbar(obshqcd[1][1:]-0.025, obshqcd[0], yerr=obshqcde, fmt='.', color="xkcd:blue",label="Test Lo", markersize='10')
