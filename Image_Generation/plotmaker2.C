...
void plotmaker2(int n=1600,TString inputfile="testing_training.root"){ // here n is dependant on number of track. For example 100 track -> 16 layers -> 100*16=1600 coordinate pairs {(x,y)->1600}
  ...
  for(int event=0; event<nentries; event++){
    ...
    gr_1->GetYaxis()->SetRangeUser(-25,25); //only works for y axis
    gr_1->GetXaxis()->SetLimits(-25,25); // only works for x axis
    ...
    string temp_iname = "images/Training/img"+str1.str()+".png";
    if(event>nentries/2 - 1) temp_iname = "images/Testing/img"+str1.str()+".png";
