...
void event_data(int ntrack=50,int hpt=0,int total_events=10, const char* opfile="test.txt"){
  ...
  for(event = 1 ; event<total_events+1;event++){  
	...
	q=-1;
      ...
      //choosing roots randomly
      rand_num = grandom->Uniform(0,1);
      if(rand_num<0.5){
	py=py;
      }
      else{
  ...
  TCanvas *c2 = new TCanvas("c2","c2",800,600); hstphi->Draw();
